...
CRdraw_texture <- function(graphics_list, texture_list, index, texture_rect = list(), target_rect = list(), angle = 0, center_point = list()  ) {
  ...
  if (index > length(texture_list$texturefile)) {
    stop("Index is out of range of \"texture_list$texturefile\".")
  }
  if ( !is.null(names(texture_rect)) && !isRectList(texture_rect)) {
    stop("Texture rect must be null or \"rect list\".")
  }
  if ( !is.null(names(target_rect)) && !isRectList(target_rect)) {
    stop("Target rect must be null or \"rect list\".")
  }
  if ( !is.null(names(center_point)) && !isPointList(center_point)) {
    stop("Center point must be null or \"point list\".")
  }
