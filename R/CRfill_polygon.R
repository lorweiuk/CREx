...
CRfill_polygon <- function(graphics_list, polygon_list, status = NULL) {
        ...
        if (length(polygon_list$y) < 1) {
          ...
          if (length(polygon_list$x) != length(polygon_list$y)) {
            ...
            if (isStatus(status)) {
              ...
              invisible( .Call("SDLfill_polygon", graphics_list, polygon_list$x, polygon_list$y, length(polygon_list$x), status, PACKAGE = "CREx") )
              
            } else {
              
              invisible( .Call("SDLfill_polygon", graphics_list, polygon_list$x, polygon_list$y, length(polygon_list$x), NULL, PACKAGE = "CREx") )
